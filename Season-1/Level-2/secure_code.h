...
bool update_setting(int user_id, const char *index, const char *value) {
    ...
    // Defensive: Check input pointers are non-null
    if (!index || !value || !accounts[user_id])
        return false;

    char *endptr;
    // Parse the setting index string
    long i = strtol(index, &endptr, 10);

    // Check: index is not empty, parses to end, no extra chars or spaces
    if (index[0] == '\0' || *endptr != '\0')
        return false;

    // Harden: index must be within bounds (no negatives, no overflow)
    if (i < 0 || i >= SETTINGS_COUNT)
        return false;

    // Now safely cast, because we've ensured i is in [0, SETTINGS_COUNT-1]
    size_t idx = (size_t)i;

    // Parse value string safely
    long v = strtol(value, &endptr, 10);

    // Check: value is not empty, parses to end, no extra chars or spaces
    if (value[0] == '\0' || *endptr != '\0')
        return false;

    // (Optionally: add value range validation here, e.g., if settings only take small ints)
